export function htmlNewUser(isAdminCreate, firstName, lastName, tempPassword, token, user_email) {
    const year = new Date().getFullYear()

    const path = "/auth/email-confirma"
    const baseUrl = process.env.URL_BASE
    const href = baseUrl + path + "?token=" + token + "&user_email=" + user_email
...
export function htmlResetPassword(token,user_email) {
    const year = new Date().getFullYear()
            ...
            <tr>
                <td align="center">
                    <table width="100%" max-width="600" cellpadding="0" cellspacing="0" style="background-color:#ffffff; padding:20px; border-radius:8px; box-shadow:0 0 10px rgba(0,0,0,0.05);">
                        <tr>
                            <td align="center" style="padding: 20px;">
                                <h2 style="color:#333333; margin-bottom:0;"> Olá caro Cliente!</h2>
                                '<p style="color:#666666; margin-top:8px;">Você acaba de solicitar uma nova senha, clique no link abaixo para continuar.</p>'
