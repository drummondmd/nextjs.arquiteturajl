        ...
        <>
            ...
            <form action={formAction} className="mt-6 max-w-3xl mx-auto p-6 space-y-6 bg-white rounded shadow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <FormField staticForm required state={state} label={"Nome"} name={"firstName"} type="text" />
                    <FormField staticForm required state={state} label={"Sobrenome"} name={"lastName"} type="text" />
                </div>

                <FormField staticForm required state={state} label={"Sexo"} name={"gender"} options={[{ label: "Masculino", value: "masculino" }, { label: "Feminino", value: "feminino" }]} as="radio" />
                <FormField staticForm required state={state} label="Data de Nascimento" as="date" name={"birthDate"} />
                <FormField staticForm required state={state} label={"CPF"} name={"cpf"} />

                {creator != "arquiteto" && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <FormField staticForm required state={state} label={"Senha"} type="password" name={"password"} />
                        <FormField staticForm required state={state} label={"Confirme sua senha"} type="password" name={"passwordConfirm"} />
